...
class MuCo(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        dataset_len = len(db.imgs.keys())
        # print("keys " , db.imgs.keys())
        perc = 10
        idxs = np.arange(dataset_len)
        np.random.seed(1)
        np.random.shuffle(idxs)
        perc_idx = idxs[:int((dataset_len * perc)/100)] 

        print("In MuCo, len(db imgs keys) ", dataset_len)
        print("In MuCo, len(perc_idx), having first 10 samples ", len(perc_idx), perc_idx[:10])

        datalist = []
        # for iid in db.imgs.keys():
        for iid in perc_idx:
            # print("In for iid = ",db.imgs.keys())
            # print("read img " , db.imgs[perc_idx[0]])
            img = db.imgs[iid]
            img_id = img["id"]
            ...
            # print(imgname.split("/"))
            # print(self.img_dir)
            # dirs = os.listdir(os.path.join(self.img_dir,'augmented_set'))
            # print("dirs ", dirs, imgname.split("/")[1])
            # if imgname.split("/")[1] not in ['1']:
            #   break
        ...
        print("Len self.datalist " , len(datalist))
        return datalist
