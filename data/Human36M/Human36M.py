...
class Human36M(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        for aid in perc_idx:
            ...
            subject_dir, subject_img = img['file_name'].split('/')[0], img['file_name'].split('/')[1]

            
            dirs = os.listdir(os.path.join(self.img_dir))
            
            if subject_dir not in dirs:
                continue
            elif subject_img not in os.listdir(osp.join(self.img_dir,subject_dir)):
                continue
            # print("self img dirg",subject_dir)
            # print("listdir ", len(os.listdir(osp.join(self.img_dir,subject_dir))))
